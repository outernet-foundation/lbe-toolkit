...
namespace Outernet.LBEToolkit.StateSynchronization
{
    ...
    public class PathIDManifest<T> where T : class, IStateNode, new()
    {
        ...
        {
            var root = new T();
            root.Initialize(new ObserveThing.ObservationContext(), new DefaultLogger() { logLevel = LogLevel.None });
        ...
        private PathData PathDataFromState(IStateNode state, T relativeTo)
        {
            ...
            while (currUpstream != relativeTo)
            {
                if (currUpstream.parent != null)
                {
                    ...
                    {
                        _keySerializerBuffer.Add(Serialization.GetSerializer(typeof(int)));
                    ...
                    {
                        _keySerializerBuffer.Add(Serialization.GetSerializer(dictionary.keyType));
                        _pathSegmentBuffer.Add("*");
                        currUpstream = currUpstream.parent;
                        continue;
                    }
